...
async function getAssetFromGithubApi(opts, assetName, downloadFolder) {
    ...
    if (proxyUrl) {
        console.info(`Proxy detected from the variables set in environment, now using proxy ${proxy} for downloading`)
        downloadOpts.agent = new ProxyUtil(proxyUrl).agent;
    } 
